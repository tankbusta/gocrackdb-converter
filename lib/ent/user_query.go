...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withCreatedTasks; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.CreatedTasks = []*Task{}
		}
		query.withFKs = true
		query.Where(predicate.Task(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CreatedTasksColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.task_created_by
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "task_created_by" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "task_created_by" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CreatedTasks = append(node.Edges.CreatedTasks, n)
		}
	}
