...
type TaskQuery struct {
	...
	withCrackedPasswords *CrackedPasswordQuery
	withFKs              bool
...
func (tq *TaskQuery) QueryCrackedPasswords() *CrackedPasswordQuery {
	query := &CrackedPasswordQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withCrackedPasswords: tq.withCrackedPasswords.Clone(),
...
// WithCrackedPasswords tells the query-builder to eager-load the nodes that are connected to
// the "cracked_passwords" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithCrackedPasswords(opts ...func(*CrackedPasswordQuery)) *TaskQuery {
	query := &CrackedPasswordQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withCrackedPasswords = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if query := tq.withCreatedBy; query != nil {
		...
		for i := range nodes {
			if nodes[i].task_created_by == nil {
				continue
			}
			fk := *nodes[i].task_created_by
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "task_created_by" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := tq.withCrackedPasswords; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Task)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.CrackedPasswords = []*CrackedPassword{}
		}
		query.withFKs = true
		query.Where(predicate.CrackedPassword(func(s *sql.Selector) {
			s.Where(sql.InValues(task.CrackedPasswordsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.task_cracked_passwords
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "task_cracked_passwords" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "task_cracked_passwords" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CrackedPasswords = append(node.Edges.CrackedPasswords, n)
		}
	}
